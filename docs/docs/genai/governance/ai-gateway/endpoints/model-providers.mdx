...
description: Supported LLM providers in MLflow AI Gateway including OpenAI, Anthropic, Google Gemini, Azure OpenAI, AWS Bedrock, and Vertex AI. Configure endpoints and use passthrough APIs.
---
...
| **DaoXE**        | Yes  | Yes        | OpenAI-compatible multi-model API    |
...
[DaoXE](https://daoxe.com) provides a single OpenAI-compatible API for many upstream LLM providers.

When creating a DaoXE endpoint:

1. Select **DaoXE** as the provider
2. Enter your DaoXE API key
3. Enter your DaoXE base URL (e.g. `https://api.daoxe.com/v1`)
4. Enter the model name as listed by your DaoXE account's `/v1/models` endpoint
...
response = client.chat.completions.create(
    model="my-daoxe-endpoint",
    messages=[{"role": "user", "content": "Hello!"}],
)
print(response.choices[0].message.content)
```

You can list the model names available to your DaoXE account by calling `/v1/models`
on the DaoXE API.
